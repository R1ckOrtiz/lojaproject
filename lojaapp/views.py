...
class AddCarroView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        carro_id = self.request.session.get("carrp_id",None)
        if carro_id:
...
class ClienteEntrarView(FormView):
    ...
    sucess_url = reverse_lazy("lojaapp:home")
    ...
    def form_valid(self, form):
        ...
        if usr is not None and usr.cliente:
            ...
            return render(self.request,self.template_name, {"form":self.form_class,"error":"USUARIO E SENHA NÃO CORRESPONDEM"})
        return super().form_valid(form)
